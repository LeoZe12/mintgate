...
export const PlateRecognizerConfig: React.FC = () => {
  const { testConnection, updateEndpoint, currentEndpoint } = usePlateRecognizer();
  ...
  const [config, setConfig] = useState({
    ...
    enabled: true, // Always enabled since we only use SDK Offline
  });
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Configuração do Plate Recognizer SDK Offline
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Badge variant="default">SDK Offline</Badge>
        </div>
